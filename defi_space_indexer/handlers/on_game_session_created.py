...
async def on_game_session_created(
    ...
    deposit_token_address = f'0x{event.payload.deposit_token:x}'
    ...
    game_start_timestamp = event.payload.game_start_timestamp
    ...
    if session:
        ...
        session.user_deposit_token_address = deposit_token_address
    ...
    await ctx.add_contract(name=contract_name, kind='starknet', address=game_session_address, typename='game_session')

    index_name = f'{contract_name}_events'
    await ctx.add_index(name=index_name, template='game_session_events', values={'contract': contract_name})

    # Get token info
    token_win_condition_name, token_win_condition_symbol, token_win_condition_decimals = await get_token_info(
        token_win_condition_address
    )
    user_deposit_token_name, user_deposit_token_symbol, user_deposit_token_decimals = await get_token_info(
    ...
    session = await models.GameSession.create(
        ...
        user_deposit_token_address=deposit_token_address,
        ...
        user_deposit_token_name=user_deposit_token_name,
        user_deposit_token_symbol=user_deposit_token_symbol,
        user_deposit_token_decimals=user_deposit_token_decimals,
        ...
        game_start_timestamp=game_start_timestamp,  # Set from event
