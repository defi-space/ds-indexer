...
async def on_token_claimed(
    ...
    token = await models.FaucetToken.get_or_none(address=token_address, faucet_address=faucet_address)
    ...
    if token:
        ...
        ctx.logger.warning(f'Token {token_address} not found in faucet {faucet_address} when creating claim event')

    ctx.logger.info(
        f'Token claimed: sender={sender_address}, token={token_address}, amount={amount}, faucet={faucet_address}'
    )
