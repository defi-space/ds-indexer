...
async def on_user_removed_from_whitelist(
    ...
    if user:
        ...
        ctx.logger.info(f'User removed from whitelist: user={user_address}, faucet={faucet_address}')
    else:
        ctx.logger.warning(f'User {user_address} not found in faucet {faucet_address} when removing from whitelist')
