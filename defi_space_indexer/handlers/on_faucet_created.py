...
async def on_faucet_created(
    ...
    await ctx.add_contract(name=contract_name, kind='starknet', address=faucet_address, typename='faucet')

    index_name = f'{contract_name}_events'
    await ctx.add_index(name=index_name, template='faucet_events', values={'contract': contract_name})
