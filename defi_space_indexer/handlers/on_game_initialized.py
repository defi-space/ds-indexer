...
async def on_game_initialized(
    ...
    user_deposit_token_address = f'0x{event.payload.user_deposit_token_address:x}'
    ...
    if session:
        ...
        session.owner = owner
        session.user_deposit_token_address = user_deposit_token_address
    ...
    session = await models.GameSession.create(
        ...
        user_deposit_token_address=user_deposit_token_address,
        ...
        game_start_timestamp=block_timestamp,  # Set start timestamp to initialization time
        ...
        game_session_index=len(factory.game_sessions_list)
        if factory_address
        else 0,  # Set the session index based on position in list
    )
    ...
    ctx.logger.info(
        ...
        f'deposit_token={user_deposit_token_address}, '
