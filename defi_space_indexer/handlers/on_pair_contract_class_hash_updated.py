...
async def on_pair_contract_class_hash_updated(
    ...
    if not factory:
        ctx.logger.warning(f"Factory {factory_address} not found when updating pair contract class hash")
        return
    ...
    ctx.logger.info(
        f"Pair contract class hash updated: factory={factory_address}, "
